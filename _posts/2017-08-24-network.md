...
| (上阶段返回) DNS server(所对应的ip?) | 具体顶级域名       | 负责该顶级域名的DNS server 域名                  | NS       |
| (上阶段返回) DNS server(所对应的ip?) | 具体次级域名       | 负责该次级域名的DNS server 域名                  | NS       |
| (上阶段返回) DNS server(所对应的ip?) | 具体主机域名       | 负责该主机域名的DNS server 域名                  | NS       |
| (上阶段返回) DNS server(所对应的ip?) | 具体主机域名       | 主机对应的ip                                     | A        |
