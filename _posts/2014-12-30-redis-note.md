...
redis事务实现了隔离性和一致性, 但是不保证原子性和持久性

* 原子性：事务中的命令要么全部被执行，要么全部都不执行, 但不保证都会成功, 如果有失败的步骤, 不影响后续执行, 不会回滚

* 持久性: 在单纯的内存模式下，事务肯定是不持久的

  <https://segmentfault.com/a/1190000002594059>
